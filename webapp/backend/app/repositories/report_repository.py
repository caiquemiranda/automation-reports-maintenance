...
from ..models.maintenance_report import MaintenanceReportDB, PartDB, MaintenanceReport, Part

class ReportRepository:
    """Repositório para operações de banco de dados com relatórios de manutenção."""
    
    @staticmethod
    def get_reports(db: Session, skip: int = 0, limit: int = 100) -> List[MaintenanceReportDB]:
        """
        ...
        Returns:
            Lista de relatórios
        """
        return db.query(MaintenanceReportDB).offset(skip).limit(limit).all()
    
    @staticmethod
    def get_report(db: Session, report_id: int) -> Optional[MaintenanceReportDB]:
        """
        ...
        """
        return db.query(MaintenanceReportDB).filter(MaintenanceReportDB.id == report_id).first()
    
    @staticmethod
    def create_report(db: Session, report: MaintenanceReport) -> MaintenanceReportDB:
        """
    ...
    @staticmethod
    def update_report(db: Session, report_id: int, report: MaintenanceReport) -> Optional[MaintenanceReportDB]:
        """
